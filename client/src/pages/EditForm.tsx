...
import { editPost } from '@src/store/requests/post.request';
...
function EditForm() {
  ...
  useEffect(() => {
    if (type === 'drawing') {
      setTitle(drawings[index!].title);
      setContent(drawings[index!].content);
    } else {
      setTitle(post?.title!);
  ...
  const handleModify = async (e: React.MouseEvent<HTMLButtonElement>) => {
    if (title.length > 50) {
      ...
      if (type === 'drawing') {
        ...
        try {
          await dispatch(editPost({ postId: post?.id!, title, content, imageKeys }));
          navigate(-1);
        } catch (err: any) {
          alert(err.message);
        }
