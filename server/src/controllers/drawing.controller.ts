...
export const createDrawing = async (req: Request, res: Response): Promise<any> => {
  ...
  if (!content) {
    logger.warn('그림내용을 입력하지 않아서 그림생성 실패하였습니다.');
    throw ApiError.BadRequest('그림 내용을 입력해주세요.');
  }
  if (content.length > 2000) {
    logger.warn('그림내용 글자를 초과해서 그림생성 실패하였습니다.');
    throw ApiError.BadRequest('그림 내용 글자수를 초과하였습니다.');
  }
  ...
  const newDrawingId = result.raw[0].id;
  ...
  const newDrawingJoinUser = await drawingRepo.joinUser(newDrawingId);
...
export const getDrawings = async (req: Request, res: Response): Promise<any> => {
  ...
  if (Number(cursor) === 0) {
    drawings = await drawingRepo.getDrawingsById(Number(userId), limit);
  } else {
    drawings = await drawingRepo.getDrawingsByCursor(Number(userId), Number(cursor), limit);
  }
...
export const deleteDrawing = async (req: Request, res: Response): Promise<any> => {
  ...
  const deletedDrawing = await drawingRepo.delete(Number(drawingId));
...
export const createComment = async (req: Request, res: Response): Promise<any> => {
  ...
  const newComment = await commentRepo.create(id, Number(drawingId), content);
...
export const updateComment = async (req: Request, res: Response): Promise<any> => {
  ...
  const updatedComment = await commentRepo.update(Number(commentId), updatedContent);

  // 위에 newComment은 user정보가 없다.
  // 응답을 보내면 새로 추가된 댓글은 user 정보가 빠진 view를 보여주게 된다.
  // 그래서. comment와 user를 결합시킨 newDrawing으로 응답을 보내준다.
  const udpatedCommentJoinUser = await commentRepo.joinUser(Number(updatedComment.id));
...
export const deleteComment = async (req: Request, res: Response): Promise<any> => {
  ...
  const deletedComment = await commentRepo.delete(Number(commentId));
...
export const createLike = async (req: Request, res: Response): Promise<any> => {
  ...
  const drawing = await getRepository(Drawing).count({ id: Number(drawingId) });
  if (!drawing) {
    logger.warn('존재하지 않는 그림에 좋아요를 추가하려고 시도합니다.');
    throw ApiError.NotFound('존재하지 않는 그림입니다.');
  }

  const newLike = await likeRepo.create(id, Number(drawingId));
...
export const createDisLike = async (req: Request, res: Response): Promise<any> => {
  ...
  const newDisLike = await disLikeRepo.create(id, Number(drawingId));
